...
When('I click the defined text link {string}', async (phrase) => {
  ...
  if (link) {
    // I think the Promise.all in the docs means something else - a wait
    // for navigation that is separate from the one triggered by the click
    await link.click();
    await scope.page.waitForNavigation({waitUntil: 'domcontentloaded'})
  } else {
    if (process.env.DEBUG) {
...
Then('an element should have the id {string}', async (phrase) => {
  const element = await scope.page.$(phrase);
